...
class IndicatorEvaluacionPreopActivity : AppCompatActivity() {
    ...
    private fun setupFilters() = with(b.filters) {
        ...
        acGranularity.setAdapter(
            ArrayAdapter(this@IndicatorEvaluacionPreopActivity, android.R.layout.simple_list_item_1, grans)
        )
        ...
        val openPicker = {
            ...
            picker.show(supportFragmentManager, "preop_range")
        }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                vm.state.collect { s ->
                    ...
                    s.data?.let { d ->
                        // KPI: minutos IA (promedio por período)
                        val minutes = (d.avgIaSeconds ?: 0.0) / 60.0
                        b.tvKpi.text = String.format(Locale.getDefault(), "IA: %.1f min", minutes)

                        val labels = d.monthly.map { it.monthLabel }
                        val entries = d.monthly.mapIndexed { i, m ->
                            Entry(i.toFloat(), ((m.avgIaSeconds ?: 0.0)/60.0).toFloat())
                        }

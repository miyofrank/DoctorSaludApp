...
class IndicatorTiempoInterpretacionActivity : AppCompatActivity() {
    ...
    private fun setupFilters() = with(b.filters) {
        ...
        acGranularity.setAdapter(
            ArrayAdapter(this@IndicatorTiempoInterpretacionActivity, android.R.layout.simple_list_item_1, grans)
        )
        ...
        val openPicker = {
            ...
            picker.show(supportFragmentManager, "tiempo_range")
        }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                vm.state.collect { s ->
                    ...
                    s.data?.let { d ->
                        // KPI: promedio segundos IA
                        val iaS = d.avgIaSeconds ?: 0.0
                        b.tvKpi.text = String.format(Locale.getDefault(), "%.1f s", iaS)
